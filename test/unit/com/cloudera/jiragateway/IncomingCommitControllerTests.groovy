...
@Mock([IncomingCommit, GatewayConfig])
class IncomingCommitControllerTests {

  void testSaveSingleCommit() {
    GatewayConfig config = new GatewayConfig(jiraProjects: "CDH,KITCHEN,OPSAPS",
            jiraHost: "foo",
            jiraUsername: "bar",
            jiraPassword: "baz",
    ...
    assert response.json == JSON.parse("{message:\"OK\"}")
    assert IncomingCommit.count() == 1
    assert IncomingCommit.get(1).jiras.size() == 1
    assert IncomingCommit.get(1).jiras.first().issueId == "KITCHEN-5260"
  }
}
